...
class Statistics:
    ...
    def get_stat_dataset(self,num_proc:int,batched:bool=False,map_fn_batch=None,map_fn=None):
        """
        ...
        if batched:
            ...
            stat_dataset = self.dataset.map(map_fn_batch,batched=batched,num_proc=num_proc)
        else:
            if map_fn:
                map_fn = self.map_fn
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config",type=str,help="Path to the config file",default="stats/config/stats_config.json")
    parser.add_argument("--num_proc",type=int,help="Path to the config file",default=mp.cpu_count())
    ...
    if args.analyze == "stat":
        ...
        stat_dataset = stat_class.get_stat_dataset(map_fn=master_map_fn,batched=True,num_proc=args.num_proc)
